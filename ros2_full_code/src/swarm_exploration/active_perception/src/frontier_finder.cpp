...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, const rclcpp::Node::SharedPtr& node) {
  ...
  cluster_min_ = declare_or_get<int>(node, "frontier.cluster_min", -1);
  cluster_size_xy_ = declare_or_get<double>(node, "frontier.cluster_size_xy", -1.0);
  cluster_size_z_ = declare_or_get<double>(node, "frontier.cluster_size_z", -1.0);
  min_candidate_dist_ = declare_or_get<double>(node, "frontier.min_candidate_dist", -1.0);
  min_candidate_clearance_ = declare_or_get<double>(node, "frontier.min_candidate_clearance", -1.0);
  candidate_dphi_ = declare_or_get<double>(node, "frontier.candidate_dphi", -1.0);
  candidate_rmax_ = declare_or_get<double>(node, "frontier.candidate_rmax", -1.0);
  candidate_rmin_ = declare_or_get<double>(node, "frontier.candidate_rmin", -1.0);
  candidate_rnum_ = declare_or_get<int>(node, "frontier.candidate_rnum", -1);
  down_sample_ = declare_or_get<int>(node, "frontier.down_sample", -1);
  min_visib_num_ = declare_or_get<int>(node, "frontier.min_visib_num", -1);
  min_view_finish_fraction_ = declare_or_get<double>(node, "frontier.min_view_finish_fraction", -1.0);
  // Cells with z below this are ignored during frontier expansion. Default
  // 0.2 matches the hard-coded legacy value; quadruped launches override to
  // stand_height-0.05 so near-ground cells aren't spuriously pruned.
  ground_clearance_ = declare_or_get<double>(node, "frontier.ground_clearance", 0.2);
  // Quadruped mode: pin viewpoint sample z to this value (0 disables).
  viewpoint_z_pin_ = declare_or_get<double>(node, "frontier.viewpoint_z_pin", 0.0);
...
void FrontierFinder::expandFrontier(
  ...
  while (!cell_queue.empty()) {
    ...
    for (auto nbr : nbrs) {
      ...
      if (pos[2] < ground_clearance_) continue;  // Remove noise close to ground
      expanded.push_back(pos);
      cell_queue.push(nbr);
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  // For quadrupeds (viewpoint_z_pin_ > 0) the viewpoint z is pinned to the
  // configured stand height so the planner doesn't propose mid-air vantage
  // points on a frontier cluster that spans a range of z values.
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      if (viewpoint_z_pin_ > 0.0) sample_pos[2] = viewpoint_z_pin_;
