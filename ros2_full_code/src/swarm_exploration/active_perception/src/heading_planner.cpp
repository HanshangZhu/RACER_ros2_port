...
void Graph::dijkstraSearch(const int& start, const int& goal, vector<YawVertex::Ptr>& path) {
  ...
  RCLCPP_ERROR(rclcpp::get_logger("active_perception"), "Dijkstra can't find path!");
  assert(false);
}
...
HeadingPlanner::HeadingPlanner(const rclcpp::Node::SharedPtr& node) {
  node_ = node;
  frontier_pub_ = node->create_publisher<sensor_msgs::msg::PointCloud2>("/heading_planner/frontier", 20);
  visib_pub_ = node->create_publisher<sensor_msgs::msg::PointCloud2>("/heading_planner/visib", 20);
  box_pub_ = node->create_publisher<visualization_msgs::msg::Marker>("/heading_planner/box", 20);

  yaw_diff_ = declare_or_get<double>(node, "heading_planner.yaw_diff", -1.0);
  lambda1_ = declare_or_get<double>(node, "heading_planner.lambda1", -1.0);
  lambda2_ = declare_or_get<double>(node, "heading_planner.lambda2", -1.0);
  half_vert_num_ = declare_or_get<int>(node, "heading_planner.half_vert_num", -1);
  max_yaw_rate_ = declare_or_get<double>(node, "heading_planner.max_yaw_rate", -1.0);
  w_ = declare_or_get<double>(node, "heading_planner.w", -1.0);
  weight_type_ = declare_or_get<int>(node, "heading_planner.weight_type", -1);
...
void HeadingPlanner::searchPathOfYaw(const vector<Eigen::Vector3d>& pts, const vector<double>& yaws,
  ...
  for (int i = 0; i < yaws.size(); ++i) {
    ...
    auto t1 = node_ ? node_->get_clock()->now() : rclcpp::Clock().now();
...
void HeadingPlanner::visualizeBox(const Eigen::Vector3d& lb, const Eigen::Vector3d& ub) {
  ...
  mk.header.stamp = node_ ? node_->get_clock()->now() : rclcpp::Clock().now();
