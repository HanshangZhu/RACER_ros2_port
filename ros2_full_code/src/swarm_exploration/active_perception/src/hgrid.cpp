...
HGrid::HGrid(const shared_ptr<EDTEnvironment>& edt, const rclcpp::Node::SharedPtr& node) {

  this->edt_ = edt;
  consistent_cost_ = declare_or_get<double>(node, "partitioning.consistent_cost", 3.5);
  consistent_cost2_ = declare_or_get<double>(node, "partitioning.consistent_cost2", 3.5);
  use_swarm_tf_ = declare_or_get<bool>(node, "partitioning.use_swarm_tf", false);
  w_first_ = declare_or_get<double>(node, "partitioning.w_first", 1.0);

  path_finder_.reset(new Astar);
  path_finder_->init(node, edt);

  grid1_.reset(new UniformGrid(edt, node, 1));
  grid2_.reset(new UniformGrid(edt, node, 2));
