...
PerceptionUtils::PerceptionUtils(const rclcpp::Node::SharedPtr& node) {
  top_angle_ = declare_or_get<double>(node, "perception_utils.top_angle", -1.0);
  left_angle_ = declare_or_get<double>(node, "perception_utils.left_angle", -1.0);
  right_angle_ = declare_or_get<double>(node, "perception_utils.right_angle", -1.0);
  max_dist_ = declare_or_get<double>(node, "perception_utils.max_dist", -1.0);
  vis_dist_ = declare_or_get<double>(node, "perception_utils.vis_dist", -1.0);
