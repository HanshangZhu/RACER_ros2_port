...
VisibilityUtil::VisibilityUtil(const rclcpp::Node::SharedPtr& node) {
  // set parameters
  min_visib_ = declare_or_get<double>(node, "visibility.visib_min", -1.0);
  max_safe_dist_ = declare_or_get<double>(node, "visibility.max_safe_dist", -1.0);
  safe_margin_ = declare_or_get<double>(node, "visibility.safe_margin", -1.0);
  max_acc_ = declare_or_get<double>(node, "visibility.max_acc", -1.0);
  r0_ = declare_or_get<double>(node, "visibility.r0", -1.0);
  wnl_ = declare_or_get<double>(node, "optimization.wnl", -1.0);
  forward_ = declare_or_get<double>(node, "visibility.forward", -1.0);
...
vector<Eigen::Vector3d> VisibilityUtil::precomputeForVisibility(
  ...
  for (int j = ctrl_pts.size() - 1; j >= visible_num_; --j) {
    ...
    if (state == 1) {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("active_perception"), "WHAT?");
    }
