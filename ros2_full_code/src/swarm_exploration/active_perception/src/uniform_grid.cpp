...
UniformGrid::UniformGrid(
    const shared_ptr<EDTEnvironment>& edt, const rclcpp::Node::SharedPtr& node, const int& level) {

  this->edt_ = edt;

  // Read min, max, resolution here
  min_[0] = declare_or_get<double>(node, "sdf_map.box_min_x", 0.0);
  min_[1] = declare_or_get<double>(node, "sdf_map.box_min_y", 0.0);
  min_[2] = declare_or_get<double>(node, "sdf_map.box_min_z", 0.0);
  max_[0] = declare_or_get<double>(node, "sdf_map.box_max_x", 0.0);
  max_[1] = declare_or_get<double>(node, "sdf_map.box_max_y", 0.0);
  max_[2] = declare_or_get<double>(node, "sdf_map.box_max_z", 0.0);
  min_unknown_ = declare_or_get<int>(node, "partitioning.min_unknown", 10000);
  min_frontier_ = declare_or_get<int>(node, "partitioning.min_frontier", 100);
  min_free_ = declare_or_get<int>(node, "partitioning.min_free", 3000);
  consistent_cost_ = declare_or_get<double>(node, "partitioning.consistent_cost", 3.5);
  w_unknown_ = declare_or_get<double>(node, "partitioning.w_unknown", 3.5);

  double grid_size = declare_or_get<double>(node, "partitioning.grid_size", 5.0);
...
void UniformGrid::updateGridData(const int& drone_id, vector<int>& grid_ids, vector<int>& parti_ids,
  ...
  for (int i = 0; i < grid_data_.size(); ++i) {
    ...
    if (!grid.is_prev_relevant_ && grid.is_cur_relevant_ && level_ > 1) {
      ...
      RCLCPP_WARN(rclcpp::get_logger("active_perception"), "Grid %d is rediscovered", i);
    }
  }
  ...
  if (!initialized_) {
    ...
    // else
    //   grid_ids = {};
    RCLCPP_WARN(rclcpp::get_logger("active_perception"), "Init grid allocation.");
    initialized_ = true;
  } else {
