...
// ViewConstraint: used to be defined in active_perception/traj_visibility.h.
// Duplicated here so bspline_opt does not pull in active_perception (which is
// not yet ported to ROS 2). When active_perception is ported, it should
// include this header instead of redefining the struct.
...
private:
  ...
  double max_vel_z_, max_acc_z_;  // z-axis caps (quadruped mode tightens these)
  double wnl_, dlmin_;
