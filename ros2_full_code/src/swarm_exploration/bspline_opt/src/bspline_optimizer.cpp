...
namespace {
template <typename T>
T declareOrGet(rclcpp::Node::SharedPtr node, const std::string& name, const T& def) {
  if (!node->has_parameter(name)) return node->declare_parameter<T>(name, def);
  return node->get_parameter(name).get_value<T>();
}
}  // namespace

void BsplineOptimizer::setParam(rclcpp::Node::SharedPtr node) {
  ld_smooth_ = declareOrGet<double>(node, "optimization.ld_smooth", -1.0);
  ld_dist_ = declareOrGet<double>(node, "optimization.ld_dist", -1.0);
  ld_feasi_ = declareOrGet<double>(node, "optimization.ld_feasi", -1.0);
  ld_start_ = declareOrGet<double>(node, "optimization.ld_start", -1.0);
  ld_end_ = declareOrGet<double>(node, "optimization.ld_end", -1.0);
  ld_guide_ = declareOrGet<double>(node, "optimization.ld_guide", -1.0);
  ld_waypt_ = declareOrGet<double>(node, "optimization.ld_waypt", -1.0);
  ld_view_ = declareOrGet<double>(node, "optimization.ld_view", -1.0);
  ld_time_ = declareOrGet<double>(node, "optimization.ld_time", -1.0);
  ld_swarm_ = declareOrGet<double>(node, "optimization.ld_swarm", -1.0);

  dist0_ = declareOrGet<double>(node, "optimization.dist0", -1.0);
  swarm_safe_dist_ = declareOrGet<double>(node, "optimization.swarm_safe_dist", -1.0);
  max_vel_ = declareOrGet<double>(node, "optimization.max_vel", -1.0);
  max_acc_ = declareOrGet<double>(node, "optimization.max_acc", -1.0);
  // Quadruped mode: tighter z-axis velocity cap so the optimizer actively
  // suppresses any vertical motion (the sim pins z anyway, but this keeps
  // the planned trajectory realistic for a ground robot).
  max_vel_z_ = declareOrGet<double>(node, "optimization.max_vel_z", max_vel_);
  max_acc_z_ = declareOrGet<double>(node, "optimization.max_acc_z", max_acc_);
  dlmin_ = declareOrGet<double>(node, "optimization.dlmin", -1.0);
  wnl_ = declareOrGet<double>(node, "optimization.wnl", -1.0);

  max_iteration_num_[0] = declareOrGet<int>(node, "optimization.max_iteration_num1", -1);
  max_iteration_num_[1] = declareOrGet<int>(node, "optimization.max_iteration_num2", -1);
  max_iteration_num_[2] = declareOrGet<int>(node, "optimization.max_iteration_num3", -1);
  max_iteration_num_[3] = declareOrGet<int>(node, "optimization.max_iteration_num4", -1);
  max_iteration_time_[0] = declareOrGet<double>(node, "optimization.max_iteration_time1", -1.0);
  max_iteration_time_[1] = declareOrGet<double>(node, "optimization.max_iteration_time2", -1.0);
  max_iteration_time_[2] = declareOrGet<double>(node, "optimization.max_iteration_time3", -1.0);
  max_iteration_time_[3] = declareOrGet<double>(node, "optimization.max_iteration_time4", -1.0);

  algorithm1_ = declareOrGet<int>(node, "optimization.algorithm1", -1);
  algorithm2_ = declareOrGet<int>(node, "optimization.algorithm2", -1);
  bspline_degree_ = declareOrGet<int>(node, "manager.bspline_degree", 3);
...
void BsplineOptimizer::optimize(Eigen::MatrixXd& points, double& dt, const int& cost_function,
  ...
  if (start_state_.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("bspline_opt"), "Initial state undefined!");
    return;
  }
  ...
  if (variable_num_ <= 0) {
    RCLCPP_ERROR(rclcpp::get_logger("bspline_opt"), "Empty varibale to optimization solver.");
    return;
  }
  ...
  comb_time = 0.0;
  plan_start_time_ = rclcpp::Clock().now().seconds();
...
void BsplineOptimizer::calcFeasibilityCost(const vector<Eigen::Vector3d>& q, const double& dt,
  ...
  for (int i = 0; i < q.size() - 1; ++i) {
    ...
    for (int k = 0; k < 3; ++k) {
      // Per-axis cap: z uses max_vel_z_ (quadruped mode tightens this so
      // the optimizer penalises vertical motion; default == max_vel_).
      const double cap = (k == 2) ? max_vel_z_ : max_vel_;
      double vd = fabs(vi[k]) - cap;
      if (vd > 0.0) {
        cost += pow(vd, 2);
  ...
  for (int i = 0; i < q.size() - 2; ++i) {
    ...
    for (int k = 0; k < 3; ++k) {
      const double cap = (k == 2) ? max_acc_z_ : max_acc_;
      double ad = fabs(ai[k]) - cap;
      if (ad > 0.0) {
        cost += pow(ad, 2);
