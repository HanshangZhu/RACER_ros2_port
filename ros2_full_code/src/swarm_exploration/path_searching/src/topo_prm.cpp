...
namespace {
template <typename T>
T declare_or_get(const rclcpp::Node::SharedPtr& node, const std::string& name,
...
void TopologyPRM::init(const rclcpp::Node::SharedPtr& node) {
  graph_.clear();
  eng_ = default_random_engine(rd_());
  rand_pos_ = uniform_real_distribution<double>(-1.0, 1.0);

  // init parameter
  sample_inflate_(0) = declare_or_get<double>(node, "topo_prm.sample_inflate_x", -1.0);
  sample_inflate_(1) = declare_or_get<double>(node, "topo_prm.sample_inflate_y", -1.0);
  sample_inflate_(2) = declare_or_get<double>(node, "topo_prm.sample_inflate_z", -1.0);
  clearance_ = declare_or_get<double>(node, "topo_prm.clearance", -1.0);
  short_cut_num_ = declare_or_get<int>(node, "topo_prm.short_cut_num", -1);
  reserve_num_ = declare_or_get<int>(node, "topo_prm.reserve_num", -1);
  ratio_to_short_ = declare_or_get<double>(node, "topo_prm.ratio_to_short", -1.0);
  max_sample_num_ = declare_or_get<int>(node, "topo_prm.max_sample_num", -1);
  max_sample_time_ = declare_or_get<double>(node, "topo_prm.max_sample_time", -1.0);
  max_raw_path_ = declare_or_get<int>(node, "topo_prm.max_raw_path", 1);
  max_raw_path2_ = declare_or_get<int>(node, "topo_prm.max_raw_path2", -1);
  parallel_shortcut_ = declare_or_get<bool>(node, "topo_prm.parallel_shortcut", false);
...
void TopologyPRM::findTopoPaths(Eigen::Vector3d start, Eigen::Vector3d end,
  ...
  graph_time = to_sec(Clock::now() - t1);
  ...
  search_time = to_sec(Clock::now() - t1);
  ...
  short_time = to_sec(Clock::now() - t1);
  ...
  prune_time = to_sec(Clock::now() - t1);
  ...
  select_time = to_sec(Clock::now() - t1);
...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {
    ...
    if (dist <= clearance_) {
      sample_time += to_sec(Clock::now() - t1);
      continue;
    }
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(vector<Eigen::Vector3d> path) {
  ...
  if (path.size() < 2) {
    RCLCPP_ERROR(rclcpp::get_logger("path_searching"), "what path? ");
    return dis_path;
  }
