...
void ObjPredictor::init() {
  auto declare_or_get_i = [&](const std::string& name, int def) {
    if (!node_handle_->has_parameter(name))
      return node_handle_->declare_parameter<int>(name, def);
    return node_handle_->get_parameter(name).get_value<int>();
  };
  auto declare_or_get_d = [&](const std::string& name, double def) {
    if (!node_handle_->has_parameter(name))
      return node_handle_->declare_parameter<double>(name, def);
    return node_handle_->get_parameter(name).get_value<double>();
  };

  obj_num_ = declare_or_get_i("prediction.obj_num", 5);
  lambda_ = declare_or_get_d("prediction.lambda", 1.0);
  predict_rate_ = declare_or_get_d("prediction.predict_rate", 1.0);
  ...
  for (int i = 0; i < obj_num_; i++) {
    ...
    auto cb = [obj_his](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg) {
      obj_his->poseCallback(msg);
    };
    auto pose_sub = node_handle_->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/dynamic/pose_" + std::to_string(i), 10, cb);
  ...
  auto period =
      std::chrono::duration<double>(1.0 / (predict_rate_ > 0 ? predict_rate_ : 1.0));
...
void ObjPredictor::predictPolyFit() {
  for (int i = 0; i < obj_num_; i++) {
    ...
    A.setZero();
    for (int k = 0; k < 3; ++k) bm[k].setZero();
...
void ObjPredictor::markerCallback(const visualization_msgs::msg::Marker::ConstSharedPtr msg) {
  ...
  if (finish_num == obj_num_) {
    // TODO(ros2-port): no direct shutdown for subscription; reset ptr instead.
    marker_sub_.reset();
  }
}
