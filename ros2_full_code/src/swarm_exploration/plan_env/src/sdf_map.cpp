...
void SDFMap::initMap(rclcpp::Node::SharedPtr node) {
  ...
  auto declare_or_get = [&](const std::string& name, auto default_val) {
    ...
    if (!node_->has_parameter(name)) {
      return node_->declare_parameter<T>(name, default_val);
    }
  ...
  mp_->resolution_ = declare_or_get("sdf_map.resolution", -1.0);
  x_size = declare_or_get("sdf_map.map_size_x", -1.0);
  y_size = declare_or_get("sdf_map.map_size_y", -1.0);
  z_size = declare_or_get("sdf_map.map_size_z", -1.0);
  mp_->obstacles_inflation_ = declare_or_get("sdf_map.obstacles_inflation", -1.0);
  mp_->local_bound_inflate_ = declare_or_get("sdf_map.local_bound_inflate", 1.0);
  mp_->local_map_margin_ = declare_or_get("sdf_map.local_map_margin", 1);
  mp_->ground_height_ = declare_or_get("sdf_map.ground_height", 1.0);
  mp_->default_dist_ = declare_or_get("sdf_map.default_dist", 5.0);
  mp_->optimistic_ = declare_or_get("sdf_map.optimistic", true);
  mp_->signed_dist_ = declare_or_get("sdf_map.signed_dist", false);
  mp_->no_drone_1_ = declare_or_get("sdf_map.no_drone_1", false);
  ...
  mp_->p_hit_ = declare_or_get("sdf_map.p_hit", 0.70);
  mp_->p_miss_ = declare_or_get("sdf_map.p_miss", 0.35);
  mp_->p_min_ = declare_or_get("sdf_map.p_min", 0.12);
  mp_->p_max_ = declare_or_get("sdf_map.p_max", 0.97);
  mp_->p_occ_ = declare_or_get("sdf_map.p_occ", 0.80);
  mp_->max_ray_length_ = declare_or_get("sdf_map.max_ray_length", -0.1);
  mp_->virtual_ceil_height_ = declare_or_get("sdf_map.virtual_ceil_height", -0.1);
  ...
  for (int i = 0; i < 3; ++i) {
    mp_->box_mind_[i] = declare_or_get("sdf_map.box_min_" + axis[i], mp_->map_min_boundary_[i]);
    mp_->box_maxd_[i] = declare_or_get("sdf_map.box_max_" + axis[i], mp_->map_max_boundary_[i]);
  }
...
bool SDFMap::getBaseCoor(const int& id, Eigen::Vector4d& transform) {
  ...
  if (iter == md_->swarm_transform_.end()) {
    RCLCPP_ERROR(rclcpp::get_logger("plan_env"), "No transform of drone %d yet", id);
    return false;
  }
