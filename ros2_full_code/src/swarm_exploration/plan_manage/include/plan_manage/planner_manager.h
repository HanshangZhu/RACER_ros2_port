...
private:
  ...
  rclcpp::Logger logger_ = rclcpp::get_logger("plan_manage");
...
private:
  ...
  // NOTE(ros2-port): VisibilityUtil lives in active_perception/traj_visibility.h
  // which conflicts with bspline_opt's duplicate ViewConstraint definition
  // (see plan_container.hpp). The upstream code never constructs visib_util_,
  // so the member is dropped entirely.
