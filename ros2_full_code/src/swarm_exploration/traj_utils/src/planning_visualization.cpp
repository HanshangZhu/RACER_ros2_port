...
PlanningVisualization::PlanningVisualization(rclcpp::Node::SharedPtr nh) {
  ...
  traj_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      "/planning_vis/trajectory", 100);
  pubs_.push_back(traj_pub_);

  topo_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      "/planning_vis/topo_path", 100);
  pubs_.push_back(topo_pub_);

  predict_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      "/planning_vis/prediction", 100);
  pubs_.push_back(predict_pub_);

  visib_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      "/planning_vis/visib_constraint", 100);
  ...
  yaw_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
      "/planning_vis/yaw", 100);
  pubs_.push_back(yaw_pub_);

  viewpoint_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>(
