...
// ROS 2 port of Map2D. Holds a running 2D occupancy grid and merges incoming
// submaps in either world-frame or local-frame poses.
...
private:
  ...
  static double yawFromQuaternion(const geometry_msgs::msg::Quaternion& q) {
    // Equivalent to tf2::getYaw, inlined so we can avoid pulling in KDL via
    // tf2_geometry_msgs (its PI symbol collides with pose_utils.h).
...
public:
  Map2D() {
    map.data.resize(0);
    map.info.origin.orientation = quaternionFromYaw(0.0);
    expandStep = 200;
    binning = 1;
    isBinningSet = false;
    updated = false;
  ...
  signed char GetOccupiedFromWorldFrame(double x, double y) {
    ...
    if (xm < 0 || xm > static_cast<int>(map.info.width) - 1 ||
        ym < 0 || ym > static_cast<int>(map.info.height) - 1)
      return 0;
    else
  ...
  void Replace(nav_msgs::msg::OccupancyGrid m) {
    ...
    if (binning > 1) {
      ...
      for (unsigned int i = 0; i < m.info.height; i++) {
        for (unsigned int j = 0; j < m.info.width; j++) {
          int val = -0xff;
  ...
  void Update(nav_msgs::msg::OccupancyGrid m) {
    ...
    double oyaw = yawFromQuaternion(m.info.origin.orientation);
